'PROGRAM GASR_PI_FRAC_09-17-2016.BAS
'THIS PROGRAM GENERATED THE DATA FOR TABLE 3 IN THE FERMAT PI FRACTION PAPER
'===========================================================================
'MODIFIED VERSION OF PROGRAM GASR_PI_FRAC_01-13-2014.BAS.
'Compiled with Power Basic/Windows Compiler 10.04.0108 (www.PowerBasic.com).

'LAST MOD 09-18-2016 ~1101 HRS EDT
'---------------------------------
...
GLOBAL PiFracIndex&& 'added 09-17-2016

GLOBAL NumIndependentGArunsPerGroup% 'added 09-17-2016

GLOBAL BestFitnessOverAllGroupsAndRuns AS EXT
...
DECLARE FUNCTION RandomIntegerPiFrac%(n%,m%) 'added 09-17-2016

DECLARE FUNCTION RandomNumPiFrac(a,b)        'added 09-17-2016

'DECLARE FUNCTION RandomIntegerPiFrac%(n%,m%,k&&) 'ORIG CODE

'DECLARE FUNCTION RandomNumPiFrac(a,b,k&&)        'ORIG CODE
...
LOCAL BESTfitnessByGroup()        AS EXT 'added 09-17-2016
LOCAL EVALSperGroup&&()                  'ditto
LOCAL AVGbestFitnessOverAllGroups AS EXT 'ditto
LOCAL VARIANCE_BestFitness        AS EXT 'ditto
LOCAL STD_DEV_BestFitness         AS EXT 'ditto

'LOCAL i%, Nd%, m%, TotalEvalsThisGroup&&, TotalEvalsOverAllGroups&&, NumGAgroups% 'ORIG CODE
LOCAL i%, Nd%, GAgroupNum%, TotalEvalsThisGroup&&, TotalEvalsOverAllGroups&&, NumGAgroups% 'added 09-17-2016
...
PiFracIndex&& = 1                 'added 09-17-2016
NumIndependentGArunsPerGroup% = 1 'ditto (MUST be 1 because PiFracIndex&& in incremented using GROUP #, not RUN #...)

'---------------------------- Test Function ---------------------------------

    CheckEarlyTerm$ = "NO" '"YES" '"NO"
...
'-------------
    BestFitnessOverAllGroupsAndRuns = -1E4200
    A$                              = ""
    NumGAgroups%                    = 100
    TotalEvalsOverAllGroups&&       = 0
    REDIM BESTfitnessByGroup(1 TO NumGAgroups%)                                                        'added 09-17-2016
    REDIM EVALSperGroup&&(1 TO NumGAgroups%)                                                           'ditto
    FOR GAgroupNum% = 1 TO NumGAgroups% 'NOTE: Option of executing NumGAgroups% > 1 groups of independent GA runs as consistency check
                                        '      if compiler built-in RND function is used for randomization instead of Pi fractions.
    PiFracIndex&& = 50*GAgroupNum%      'Added 09-17-2016 to choose a different set of Pi Fractions for each group (completely arbitrary)
                                        '      with NumIndependentGArunsPerGroup% = 1 in SUB RunGASR()
        CALL RunGASR(BestFitnessAllRuns,GASRseedChromo(),FunctionName$,Nd%,XiMin(),XiMax(),TotalEvalsThisGroup&&,GAgroupNum%,_
                     NumGAgroups%,LastGenNum&)
        IF BestFitnessAllRuns >= BestFitnessOverAllGroupsAndRuns THEN BestFitnessOverAllGroupsAndRuns = BestFitnessAllRuns
        BESTfitnessByGroup(GAgroupNum%) = BestFitnessAllRuns    'added 09-17-2016
        EVALSperGroup&&(GAgroupNum%)    = TotalEvalsThisGroup&& 'ditto
        TotalEvalsOverAllGroups&&       = TotalEvalsOverAllGroups&& + TotalEvalsThisGroup&&
'        Coords$ = "" : FOR i% = 1 TO Nd% : Coords$ = Coords$+SPACE$(30)+"i ="+STR$(i%)+_ 'this & next two lines commented out 09-17-2016
 '                 ":   "+STR$(ROUND(GASRseedChromo(i%),6)) + $CRLF : NEXT i%
  '      A$ = A$+"GA group #"+STR$(GAgroupNum%)+": Best Fitness = "+STR$(BestFitnessAllRuns,12)+"  @  coords:"+$CRLF+$CRLF+Coords$
    NEXT GAgroupNum%

    AVGbestFitnessOverAllGroups = 0##
    FOR GAgroupNum% = 1 TO NumGAgroups% 'following code added 09-17-2016
        AVGbestFitnessOverAllGroups = AVGbestFitnessOverAllGroups + BESTfitnessByGroup(GAgroupNum%)
    NEXT GAgroupNum%
    AVGbestFitnessOverAllGroups = AVGbestFitnessOverAllGroups/NumGAgroups%
    VARIANCE_BestFitness = 0##
    FOR GAgroupNum% = 1 TO NumGAgroups%
        VARIANCE_BestFitness = VARIANCE_BestFitness + (BESTfitnessByGroup(GAgroupNum%)-AVGbestFitnessOverAllGroups)^2
    NEXT GAgroupNum%
    VARIANCE_BestFitness = VARIANCE_BestFitness/NumGAgroups%
    STD_DEV_BestFitness  = SQR(VARIANCE_BestFitness)

    A$ = STR$(Nd%)+" "+FunctionName$+":"+$CRLF+$CRLF+_
         "  #GA groups = "+STR$(NumGAgroups%)+$CRLF+_
         "  # indep runs/group = "+STR$(NumIndependentGArunsPerGroup%)+$CRLF+_
         "  Total evals all grps = "+STR$(TotalEvalsOverAllGroups&&)+$CRLF+_
         "  Avg evals per group = "+STR$(TotalEvalsOverAllGroups&&/NumGAgroups%)+$CRLF+_
         "  BEST Overall Fitness = "+STR$(BestFitnessOverAllGroupsAndRuns)+$CRLF+_
         "  AVG Best Fitness   = "+STR$(AVGbestFitnessOverAllGroups)+$CRLF+_
         "  Std Deviation      = "+STR$(STD_DEV_BestFitness)+$CRLF
    MSGBOX(A$) 'end code added 09-17-2016

'    MSGBOX("in PBMAIN, "+REMOVE$(STR$(Nd%),ANY" ")+"D "+FunctionName$+_  'this & next two lines commented out 09-17-2016
...
'Creates a Group of Independent GA runs and returns Best Fitness and corresponding Chromosome across all runs in all groups.
...
'LOCAL RunNumber%, GenNum&, NumGenerations&, NumChromos%, NumIndependentGArunsPerGroup% 'commented out 09-17-2016
LOCAL RunNumber%, GenNum&, NumGenerations&, NumChromos% 'added 09-17-2016

'---------------------------------------------------
'NumIndependentGArunsPerGroup% =    1 'commented out & added as a GLOBAL variable 09-17-2016
NumGenerations&                =   99 'with NumGAgroups%=100 this and next value -> 20,000,000 fnc evals for direct comparison with v-PSO
                                      'note - '99' is actually 100 gens because generation numbering starts with zero
NumChromos%                    = 2000 'MUST BE EVEN #
'---------------------------------------------------
...
'   COMMENT 09-17-2016: Seed Chromo is created but never used.  Probably intended for future use in origibal version...
    END IF

'   THIS SECTION OF CODE COMMENTED OUT 09-17-2016
...
'            A$=   "     ######          ######.#####"
 '       FOR GenNum& = 0 TO LastGenNum& : PRINT #N%, USING$(A$,GenNum&,BestFitnessVsGenThisRun(GenNum&)) : NEXT GenNum&
  '  CLOSE #N%
'   END SECTION OF CODE COMMENTED OUT 09-17-2016
...
'           k&& = ChromoNum%*i% 'commented out 09-17-2016
'           Chromos(ChromoNum%,i%,0) = RandomNumPiFrac(XiMin(i%),XiMax(i%),k&&) 'ORIG CODE
            Chromos(ChromoNum%,i%,0) = RandomNumPiFrac(XiMin(i%),XiMax(i%))     'added 09-17-2016
        NEXT i%
    NEXT ChromoNum%
...
'   k&& = ChromoNum%+GenNum& 'commented out 09-17-2016
    IF RandomNumPiFrac(0##,1##) =< CrossOverProbability THEN     'Create 2 new chromos this gen from children 'ADDED 09-17-2016
'   IF RandomNumPiFrac(0##,1##,k&&) =< CrossOverProbability THEN 'Create 2 new chromos this gen from children 'ORIG CODE
                                                                 'whose parents are from previous generation.

'STEP (B): SELECT TWO DIFFERENT PARENT CHROMOS FROM _PREVIOUS_ GENERATION
'   k&& = ChromoNum% 'commented out 09-17-2016
    DO
        s%  = RandomIntegerPiFrac%(1,NumChromos%)     : t% = RandomIntegerPiFrac%(1,NumChromos%) 's, t are parent chromo #s       'added 09-17-2016
'       s%  = RandomIntegerPiFrac%(1,NumChromos%,k&&) : t% = RandomIntegerPiFrac%(1,NumChromos%,k&&+2) 's, t are parent chromo #s 'ORIG CODE
...
'   ------------------------------------ Choose Two Best Child Chromos ---------------------------------------
    ...
    ChildChromoFitness(1) = ObjectiveFunction(b1(),Nd%,ChromoNum%,GenNum&,FunctionName$)' : INCR NevalThisRun&& 'commented out 09-17-2016
    ChildChromoFitness(2) = ObjectiveFunction(b2(),Nd%,ChromoNum%,GenNum&,FunctionName$)' : INCR NevalThisRun&& 'ditto
    ChildChromoFitness(3) = ObjectiveFunction(b3(),Nd%,ChromoNum%,GenNum&,FunctionName$)' : INCR NevalThisRun&& 'ditto
    ChildChromoFitness(4) = ObjectiveFunction(b4(),Nd%,ChromoNum%,GenNum&,FunctionName$)' : INCR NevalThisRun&& 'ditto
...
'   k&& = ChromoNum%+GenNum& 'commented out 09-17-2016
'   IF RandomNumPiFrac(0##,1##,k&&) =< MutationProbability THEN 'mutate chromos, otherwise don't. 'ORIG CODE
    IF RandomNumPiFrac(0##,1##) =< MutationProbability THEN 'mutate chromos, otherwise don't.     'added 09-17-2016
...
'   k&&                   = GenNum& 'commented out 09-17-2016
'   ElitistChromoNumber1% = RandomIntegerPiFrac%(1,NumChromos%,k&&) 'ORIG CODE
    ElitistChromoNumber1% = RandomIntegerPiFrac%(1,NumChromos%)     'added 09-17-2016
...
'   m&& = ChromoNum%+GenNum& 'commented out 09-17-2016
'   K%  = RandomIntegerPiFrac%(1,Nd%,m&&) 'randomly select element index K for mutation 'ORIG CODE
    K%  = RandomIntegerPiFrac%(1,Nd%)     'randomly select element index K for mutation 'added 09-17-2016
...
FUNCTION RandomNumPiFrac(a,b) 'Returns random number X, a=< X < b.      'added 09-17-2016
'FUNCTION RandomNumPiFrac(a,b,k&&) 'Returns random number X, a=< X < b. 'ORIG CODE
'    RandomNumPiFrac = a + (b-a)*PiFractions(k&&) 'ORIG CODE
    RandomNumPiFrac = a + (b-a)*PiFractions(PiFracIndex&&) 'added 09-17-2016
    PiFracIndex&& = PiFracIndex + 3 'increment index by 3 (entirely arbitrary)
    IF PiFracIndex > NumPiFractions&& THEN PiFracIndex&& = 2 'arbitrary
END FUNCTION 'RandomNumPiFrac()

'------------------------------

FUNCTION RandomIntegerPiFrac%(n%,m%) 'Returns random integer I, n=< I =< m.      'added 09-17-2016
'FUNCTION RandomIntegerPiFrac%(n%,m%,k&&) 'Returns random integer I, n=< I =< m. 'ORIG CODE
'    RandomIntegerPiFrac% = n% + (m%-n%)*PiFractions(k&&) 'ORIG CODE
    RandomIntegerPiFrac% = n% + (m%-n%)*PiFractions(PiFracIndex&&)
    PiFracIndex&& = PiFracIndex + 4 'increment index by 4 (entirely arbitrary)
    IF PiFracIndex > NumPiFractions&& THEN PiFracIndex&& = 7 'arbitrary
